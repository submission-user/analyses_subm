...
save(df_all_reduced_noDIF,
     file = "df_all_reducednoDIF_100x100nullcases_cmldif.Rdata")
...
# some estimations deliver overly large estimates
## -> check which (cases for n=50)
max_estimates <- unlist(lapply(deltapars,function(x)max(abs(x))))

max_estimates[order(max_estimates, decreasing = TRUE)][1:36]
cases_extreme_est <- which(max_estimates > 10)
pars[cases_extreme_est,]
## 36 above 60, all others below 1.5
## all from n=50, nvars = 21,

max_estimates[cases_extreme_est]

results_extreme <- res_foreach[cases_extreme_est]

# rowSums of extreme cases
lapply(1:length(cases_extreme_est), 
       function(x)rowSums(results_extreme[[x]]$scenario$data))
# colSums of extreme cases
lapply(1:length(cases_extreme_est), 
       function(x)colSums(results_extreme[[x]]$scenario$data))
# cannot find reason for extreme cases
