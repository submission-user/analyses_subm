####
#
# check scenarios for DIFlasso (times and detection)
# to compare with cmlDIFlasso
# Split up into groups of 2500 for computational reasons
#
####
...
no_of_cores = 31  # 31 cores to lower RAM-burden
nlambda_wrap = 21

source("find_finished_sample_numbers.R")
pars <- pars[numbers_of_missing_samples,]
seeds <- seeds[numbers_of_missing_samples]

subsamples <- rep(1:4, length.out =length(numbers_of_missing_samples))

pars1 <- pars[subsamples==1,]
seeds1 <- seeds[subsamples==1]

pars2 <- pars[subsamples==2, ]
seeds2 <- seeds[subsamples==2]

pars3 <- pars[subsamples==3, ]
seeds3 <- seeds[subsamples==3]

pars4 <- pars[subsamples==4, ]
seeds4 <- seeds[subsamples==4]
...
# pars3 <- pars[5000 + (1:2500), ] # is now being calculated above
...
# pars4 <- pars[7500 + (1:2500), ]  # is now being calculated above
...
## prior to this: copy all files into folder "result_files - copied from all tries"

all_result_files_names <- list.files("result_files - copied from all tries/")
# sort by number:
...
all_result_files_names_sorted_fullpath <- sapply(all_result_files_names_sorted, function(x) paste("result_files - copied from all tries/",x,sep=""))
...
res_foreach <- lapply(all_result_files_names_sorted_fullpath, load_and_return)
...
df_all_reduced_DIFlasso_null_100x100_samples <- tbl1
save(df_all_reduced_DIFlasso_null_100x100_samples,
     file = "df_all_reduced_DIFlasso_null_100x100_samples.Rdata")
...
# plot all 10 BIC trajectories
for (i in 10:1) plot(x=res_foreach[[i]]$result$lambda,y=res_foreach[[i]]$result$BIC, 
                      main = paste("n =", df_t_and_scen$scenario.n[i],
                                   ", I =", df_t_and_scen$scenario.I[i],
                                   ", nvars =", df_t_and_scen$scenario.nvars[i],
                                   ",       i =",i))

## check how many of the 100x100 scenarios deliver no selected parameters
# via delta estimates
