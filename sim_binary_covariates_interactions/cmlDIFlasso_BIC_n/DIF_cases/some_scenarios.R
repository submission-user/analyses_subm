...
#
#####
# source("cml_lasso_win_lbfgs_optimized_temp.R")
# source("wrap_cml_lasso_lbfgs_optimized_temp.R")

library(doSNOW) # load first for progress bar
library(cmlDIFlasso2)
# source("cml_lasso_win_lbfgs.R")
# source("cml_lasso_parallel_win_lbfgs.R")
# source("session_all.R")
...
cl<-makeCluster(no_of_cores,type="SOCK")  # Cluster for snow
...
source("func_info_crit.R")

save(res_foreach,file = "results_cmlDIFlasso_opt_100x100scenarios_BIC_n.Rdata")
# load("results_cmlDIFlasso_opt_100x100scenarios.Rdata")
...
save(df_t_and_scen_cmlDIFlasso_opt,
     file="df_t_100x100cases_cmldif_opt_tolerancedf_0_BIC_n.Rdata")
######
#
# inspecting the data
#
...
save(df_all_reduced_cmlDIFlasso_DIF_100samples,
     file = "df_all_reduced_cmlDIFlasso_DIF_100samples_BIC_n.Rdata")
