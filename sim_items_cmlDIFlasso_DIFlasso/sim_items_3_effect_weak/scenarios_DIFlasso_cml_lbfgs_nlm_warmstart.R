...
# add smaller and null effects in other simulation
...
delta_help <- -.5*(matrix(c( -0.8,0.6,0,0,0.8, 
                         0,0.8,-0.7,0,0.7,
                         0.6,0,0.8,-0.8,0,
                         0,0,0.8,0.7,-0.5),5,4))
...
seeds <- 2999:(2999+5*repetitions-1)
no_of_cores = 59
nlambda_wrap = 21
...
res_nlm_small_effects <- res_foreach
save(res_foreach,file = "results_nlm_small_effects.Rdata")
...
# do.call(c,lapply(1:500,function(x)res_foreach[[x]]$scenario$seed))
# 
# res_nlm_warm_start_small_effects <- res_foreach
# save(res_foreach,file = "results_nlm_warm_start_small_effects.Rdata")
# 
...
res_DIFlasso_grouped_ordered_small_effects <- res_foreach
save(res_foreach,file = "results_DIFlasso_grouped_ordered_small_effects.Rdata")
