...
# compare DIFlasso, cmlDIFlasso nlm, cmlDIFlasso nlm warmstart
#
#####
...
delta_help <- -(matrix(c( -0.8,0.6,0,0,0.8, 
                         0,0.8,-0.7,0,0.7,
                         0.6,0,0.8,-0.8,0,
                         0,0,0.8,0.7,-0.5),5,4))
...
seeds <- 1999:(1999+5*repetitions-1)
no_of_cores = 59
nlambda_wrap = 21
...
# do.call(c,lapply(1:500,function(x)res_foreach[[x]]$scenario$seed))
# 
# res_nlm_warm_start <- res_foreach
# save(res_foreach,file = "results_nlm_warm_start.Rdata")
# 
...
res_DIFlasso_grouped_ordered <- res_foreach
save(res_foreach,file = "results_DIFlasso_grouped_ordered.Rdata")
