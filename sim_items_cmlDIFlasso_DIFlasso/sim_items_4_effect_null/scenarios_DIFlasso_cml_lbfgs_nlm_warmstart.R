...
delta_help <- 0*(matrix(c( -0.8,0.6,0,0,0.8, 
                         0,0.8,-0.7,0,0.7,
                         0.6,0,0.8,-0.8,0,
                         0,0,0.8,0.7,-0.5),5,4))
...
seeds <- 3499:(3499+5*repetitions-1)
no_of_cores = 59
nlambda_wrap = 21


####
# start parallel execution separately for DIFlasso, cmlDIFlasso nlm and nlm warm start (possibly also lbfgs or GPCMlasso)
############
...
#                                    .options.snow = opts,
#                                    .inorder=TRUE) %dopar% {
#                                      wrap_cml_lasso_lbfgs_optimized_temp(n, I, 5, seed, nlambda_wrap,
...
# 
# res_lbfgs_null_effects <- res_foreach
# save(res_foreach,file = "results_lbfgs_null_effects.Rdata")
# 
# time_full_lbfgs <- time_full
...
res_nlm_null_effects <- res_foreach
save(res_foreach,file = "results_nlm_null_effects.Rdata")
...
# do.call(c,lapply(1:500,function(x)res_foreach[[x]]$scenario$seed))
# 
# res_nlm_warm_start_null_effects <- res_foreach
# save(res_foreach,file = "results_nlm_warm_start_null_effects.Rdata")
# 
...
res_DIFlasso_grouped_ordered_null_effects <- res_foreach
save(res_foreach,file = "results_DIFlasso_grouped_ordered_null_effects.Rdata")
